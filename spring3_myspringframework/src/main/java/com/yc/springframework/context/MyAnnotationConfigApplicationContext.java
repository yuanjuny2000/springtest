...
public class MyAnnotationConfigApplicationContext implements MyApplicationContext{
    ...
    public void register(Class<?>[] componentClasses) throws Exception {
        ...
        for(Class cl:componentClasses){
            //请实现这个里面的代码
            //源码1：  只实现 IOC,MyPostConstruct  MyPreDestroy
            ...
            //处理  basePackages  基础包下的所以的托管bean
    ...
    private void handleDi(Map<String,Object> beanMap)throws Exception{
    ...
    /**
     * 处理manageBeanClasses 所有的Class类，筛选出所有的@Component @Server, @Repository 的类，并实例化，存到beanMap中
    ...
    /**
     * 查找 file 下面及子包所以的要托管的class，存到一个Set( managedBeanClasses )中
     * @param file
     * @param basePackage
     * @throws Exception
     */
        private void findClassInPackages(String file,String basePackage)throws Exception{
            ...
            for(File  cf: classFiles){
                if(cf.isDirectory()){
                    ...
                    findClassInPackages(cf.getAbsolutePath(),basePackage);
                }else{
                    //加载 cf 作为 class文件
                    URL[] urls=new URL[]{};
    ...
    /**
     * 处理MyAppConfig配置中的@Bean注解  完成IOC操作
     * @param cls
     * @param obj
     * @throws InvocationTargetException
     * @throws IllegalAccessException
    ...
    private void handleAtMyBean(Class cls, Object obj) throws InvocationTargetException, IllegalAccessException {
        ...
        for(Method m:ms){
            if(m.isAnnotationPresent(MyBean.class)){
                ...
                handPostConstruct(o,o.getClass());  //o 在这里指  HelloWorld对象  o.getClass() 它的反射对象
     ...
     * @param cls
     */
    private void handPostConstruct(Object o, Class<?> cls) throws InvocationTargetException, IllegalAccessException {
        Method[] ms=cls.getDeclaredMethods();
        for(Method m:ms){
